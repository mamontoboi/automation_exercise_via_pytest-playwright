...
class LoginOrSignupPage:
    ...
    @allure.step("Attempt login with an intentionally incorrect password")
    def login_via_wrong_password(self, user: User):
    ...
    def fill_account_details(self, user: User):
        ...
        self.page.locator("#days").select_option(birth_date[0])
        self.page.locator("#months").select_option(birth_date[1])
        self.page.locator("#years").select_option(birth_date[2])
        self.page.get_by_role(**self.FIRST_NAME_INPUT).fill(user.first_name or user.name.split()[0])
        self.page.get_by_role(**self.LAST_NAME_INPUT).fill(user.last_name or user.name.split()[-1])
        self.page.get_by_role(**self.ADDRESS_INPUT).fill(user.address or "")
    ...
    @allure.step("Create the new account")
    ...
    @allure.step("Verify the authentication error message")
    ...
    @allure.step("Verify the user is logged out")
    ...
    @allure.step("Verify the browser validation popup message")
    ...
    @allure.step("Verify the email field validation message")
